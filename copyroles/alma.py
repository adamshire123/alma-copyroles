...
class AlmaClient:
    def __init__(
        self, api_key, base_url="https://api-na.hosted.exlibrisgroup.com/almaws/v1/"
    ):
    ...
    def update_alma_roles(self, roles, user):
        ...
        user["user_role"] = (
            roles  # the list of roles in the user object is called 'user_role' (yes, singular)
        )
        endpoint = "users/" + quote(user["primary_id"])
        response = requests.put(
            urljoin(self.base_url, endpoint), headers=self.headers, json=user
        )
        response.raise_for_status()
