...
def main():
    ...
    try:
        alma_environment = alma_client.get_alma_environment()
    except requests.exceptions.HTTPError as e:
        exit(e)
    confirm_environment = input(
        f"You are working in the \033[1m{alma_environment.upper()}\033[0m environment. Continue? [y/n]"
    )
    if confirm_environment.lower() == "y":
        source_user = alma_client.get_alma_user(args.source_user)
        dest_user = alma_client.get_alma_user(args.dest_user)
        source_roles = source_user.get("user_role")
        continue_copy = input(
            f"This will copy roles from \033[1m{source_user['primary_id']}\033[0m to \033[1m{dest_user['primary_id']}\033[0m. Continue and review roles? [y/n]"
        )
        if continue_copy.lower() == "y":
            ...
            do_copy = input(
                "Copy these roles? WARNING: This operation cannont be undone. [type 'copy-roles' to continue] "
            )
